				...
				<div class="col-lg-12">
					<div class="panel panel-default">
						<div class="panel-heading">
							<h2><i class="fa fa-table red"></i><span class="break"></span><strong><?php if($_SESSION['userotype']= 2): ?>普通会员<?php elseif($_SESSION['userotype']= 4): ?>经纪人<?php endif; ?>列表</strong></h2>
							<div style="display: inline-block; margin-left: 20px">
								<form id="cuntomerfrom"  method="post" action="<?php echo U('Ordinary/agentlist');?>">
						...
						<div class="panel-body">
							<table class="table table-striped table-bordered bootstrap-datatable datatable">
							  ...
							  <?php if(is_array($ulist)): $i = 0; $__LIST__ = $ulist;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
										<td style="text-align:center"><?php if($_SESSION['userotype']= 2): ?>普通会员<?php elseif($_SESSION['userotype']= 4): ?>经纪人<?php endif; ?></td>
	                                    <td style="text-align:center"><?php echo ($vo["username"]); ?></td>
	                                    <td style="text-align:center"><?php echo ($vo["address"]); ?></td>
										<td style="text-align:center"><?php echo (date('Y-m-d H:i',$vo["utime"])); ?></td>
										<td style="text-align:center">
											<a class="btn btn-info" href="<?php echo U('Ordinary/agentadd');?>?uid=<?php echo ($vo['uid']); ?>">
												<i class="fa">修改</i>                                            
											</a>
											<?php if($vo["num"] > 0): ?><a class="btn btn-info" href="<?php echo U('Ordinary/agentdown');?>?uid=<?php echo ($vo['uid']); ?>">
												<i class="fa">下级</i> 
											</a><?php endif; ?>
											<a class="btn btn-info" href="#">
												<i class="fa" onclick="showpre(<?php echo ($vo["uid"]); ?>)">详情</i> 
											</a>
											<a class="btn btn-danger" href="<?php echo U('Ordinary/agentdel');?>?uid=<?php echo ($vo['uid']); ?>">
												<i class="fa">删除</i> 
											</a>
										</td>
...
<script>
	function showpre(uid)
	{
		...
		$.ajax({
			type: "post",
			url:"<?php echo U('Ordinary/showpre');?>",
