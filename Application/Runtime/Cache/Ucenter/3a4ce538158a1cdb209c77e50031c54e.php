						...
						<div class="panel-body lists">
							<table class="table table-striped table-bordered bootstrap-datatable datatable">
							  ...
							  <?php if(is_array($ulist)): $k = 0; $__LIST__ = $ulist;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($k % 2 );++$k;?><tr>
									...
									<td>
										<?php if($vo["agenttype"] == 1): ?><a class="btn btn-info">
												<i class="fa fa-edit" onclick="changestatusok(<?php echo ($vo["uid"]); ?>)">通过</i>
											</a>
											<a class="btn btn-info"> 
												<i class="fa fa-edit" onclick="changestatusno(<?php echo ($vo["uid"]); ?>)">拒绝</i>
											</a>
										<?php elseif($vo["agenttype"] == 0): ?>
											已拒绝
										<?php else: ?>
											已通过<?php endif; ?>
									</td>
...
<script>
	function changestatusok(uid)
	{
		...
		$.ajax({  
			type: "post",  
			url: "<?php echo U('Customer/verifyeditok');?>",    
			data:{"uid":uid},
	...
	function changestatusno(uid)
	{
		...
		$.ajax({  
			type: "post",  
			url: "<?php echo U('Customer/verifyeditno');?>",    
			data:{"uid":uid},
			success: function(data) {
				alert('审核不通过！');
				window.location.href = window.location.href;   
