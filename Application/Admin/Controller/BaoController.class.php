...
class BaoController extends Controller {
	public function olist(){
		$ty = M("webconfig")->where("id =1")->getField("isopen");
		if(!$ty){
			return true;
		}
		//获取所有没有平仓的订单
		...
		//获取最新产品价格
		$yprice = M("catproduct")->where("cname='CAD-LME'")->getField('ask');//油价
		$byprice = M("catproduct")->where("cname='XAGUSDOZ SC-FX'")->getField('ask');//白银价
		if(($yprice <= 0) || ($byprice <= 0))
		{
			return false;
		}
		//设置盈亏比，爆仓
		foreach($order as $k => $v){
			...
			/*if($cid == 1){
				$this->gefee($oid,$uid,$shengmoney,$gefee,$surplus,$ploss,$overday1,$buytime1,$yprice);
				continue;
			}else if($cid == 2){
				$this->gefee($oid,$uid,$shengmoney,$gefee,$surplus,$ploss,$overday1,$buytime1,$byprice);
				continue;
			}*/
		...
		if(!$overday1){
			if($overday > 0 ){
				...
				if($newd['balance'] < 0){
					...
					// echo M("accountinfo")->getLastsql();die;
