	...
	public function upbalance(){
		...
		if($isverified=="1"){
			if($type == "充值")
			{
				...
				$isver=$balance->where('bpid='.$bpid)->setField(array('isverified'=>'1','remarks'=>$remarks,'cltime'=>$cltime));//1是同意	
				$date=$accountinfo->where('uid='.$userid)->find();
				$date['frozen']=$date['frozen']-$rebpprce;
				$accountinfo->where('uid='.$userid)->save($date);		
			}			
			...
			if($type == "充值"){
				...
				$tixianshibai=$accountinfo->where('uid='.$userid)->find();
				$tixian['balance']=$tixianshibai['balance']+$rebpprce;
				$tixian['frozen']=$tixianshibai['frozen']-$rebpprce;
				$accountinfo->where('uid='.$userid)->save($tixian);
