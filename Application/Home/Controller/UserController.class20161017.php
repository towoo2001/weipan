...
class UserController extends Controller
{
	...
	public function oauth2(){
        // 　　include_once('Class/Weixinsdk.php');
			// die;
...
private function Mheader($type){
   ...
   public function reg()
    ...
    public function rechargelist(){
        ...
        $data = $balance->field('bpid,bpprice,remarks,bptime')->where($where)->select();
    ...
    public function outsmsverify()
    {
		$phone = $_REQUEST['pho'];
        $code = $this->mescontent();
		$con = "你正在修改用户密码，验证码为".$code."！[米盘微盘]";
        $row = sendsms($phone,$con);
        if($row)
		{
