...
class UserController extends Controller
{
    ...
    $this->gopayNotice_url="http://" . $_SERVER['HTTP_HOST'] . "/index.php/Home/User/gopayNotice";
	...
	public function oauth2(){
			...
			// $wxpay=new \Weixinpay();
            $weixin=new \Weixinsdk("wx073b4f54182001f2", "d2a26aca83a818e8f5cfc859bfc13d76");
            if (isset($_GET['code'])){
                ...
                if ($res['openid']){
                    ...
                    session('wxopenid',$res['openid']);

                    $userinfo = M('userinfo');
                    $userinfoid=$userinfo->where("openid='".$res['openid']."'")->find();

                    if($userinfoid['openid']) {
                        session('uid', $userinfoid['uid']);          // 当前用户id
                        session('husername', $userinfoid['username']);   // 当前用户昵称
                        session('openid', $userinfoid['openid']);

                        $this->success('授权成功！正在登陆...',U('Index/index'));

                    }
                }else{
                    $this->error('授权出错,请重新授权!',"login");
                }
    ...
    { 
		...
		if($wxopenid && I('post.password') && I('post.username'))
		{
			// 实例化Login对象
			$user = D('userinfo');
			$where = array();
			$where['username'] = I('post.username');
			$result = $user->where($where)->field('uid,username,upwd,utime,openid,vertus,ustatus')->find();
			if($result['ustatus']){
				$this->error('对不起,由于您恶意操作,帐户已被冻结,稍后请联系客服,谢谢!');
			}
			if ($result['upwd'] === md5(I('post.password') )) {
				...
				session('openid', $result['openid']);   // 当前openid
				...
				if(!$result['openid'])
				{
                    ...
                    //得到微信用户写到数据库---20161020

                        $url =file_get_contents( "https://api.weixin.qq.com/cgi-bin/token?grant_type=client_credential&appid=wx073b4f54182001f2&secret=d2a26aca83a818e8f5cfc859bfc13d76");
                        $result = json_decode($url, true);
                        $access_token = $result["access_token"];
                    $wxuser =file_get_contents( "https://api.weixin.qq.com/cgi-bin/user/info?access_token=".$access_token."&openid=".$map['openid']."&lang=zh_CN");
                    $wxlist=json_decode($wxuser,true);
                   // echo "https://api.weixin.qq.com/cgi-bin/user/info?access_token=".$access_token."&openid=".$map['openid']."&lang=zh_CN";
                   // echo '<br/>';
                   // dump($wxlist);exit;
                    $map['nickname']=$wxlist['nickname'];
                    $map['portrait']=$wxlist['headimgurl'];

                    $user->where($where1)->save($map);
				}elseif($result['openid'] != $wxopenid){
					session(null);
					$this->error('帐户绑定微信和存储信息不匹配!');
				}
				$this->redirect('Index/index');
			} else {
				$this->error('登录失败,密码不正确!');
			}
		}else if($wxopenid && I('post.password')){
			// 实例化Login对象
			$user = D('userinfo');
			$where = array();
			$where['openid'] = $wxopenid;
			$result = $user->where($where)->field('uid,username,upwd,utime,openid,vertus')->find();
			if (($result['upwd'] === md5(I('post.password'))) && ($result['openid'] == $wxopenid)) {
				if($result['vertus'] != 1)
				{
					$this->error('对不起,你还没通过平台审核，不能登录!');
				}
				...
				session(null);
				$this->error('登录失败,用户名或密码不正确，或该微信号和软件绑定帐号不一致！');
			}
		}elseif(I('post.username') && I('post.password') && !$wxopenid){
				// 实例化Login对象
				$user = D('userinfo');
				$where = array();
				$where['username'] = I('post.username');
    ...
    {//国付宝
        ...
        $feeAmt = 0;
        $currencyType = 156;
        $frontMerUrl =  $this->gopayNotice_url;
        $backgroundMerUrl = $this->reapalReturn_url;
        $tranDateTime = $order_time;
        $virCardNoIn = '0000000002000007889';
       // $virCardNoIn = '0000000002000000257';
        //注意调试生产环境时需要修改这个值为生产参数

        $tranIP = $_SERVER["REMOTE_ADDR"];
        $isRepeatSubmit = '0';  //禁止重复提交
        ...
        $signStr='version=['.$version.']tranCode=['.$tranCode.']merchantID=['.$merchantID.']merOrderNum=['.$merOrderNum.']tranAmt=['.$tranAmt.']feeAmt=['.$feeAmt.']tranDateTime=['.$tranDateTime.']frontMerUrl=['.$frontMerUrl.']backgroundMerUrl=['.$backgroundMerUrl.']orderId=[]gopayOutOrderId=[]tranIP=['.$tranIP.']respCode=[]gopayServerTime=['.$gopayServerTime.']VerficationCode=[weiltbot1234567]';
        ...
        $d['merOrderNum'] = $merOrderNum;
        $d['tranAmt'] = $tranAmt;
        $d['feeAmt'] = $feeAmt;
        $d['currencyType'] = $currencyType;
        $d['frontMerUrl'] = $frontMerUrl;
        $d['backgroundMerUrl'] = $backgroundMerUrl;
        $d['tranDateTime'] = $tranDateTime;
        $d['virCardNoIn'] = $virCardNoIn;
        //注意调试生产环境时需要修改这个值为生产参数

        $d['tranIP'] = $tranIP;
        $d['isRepeatSubmit'] = $isRepeatSubmit;  //禁止重复提交
        $d['goodsName'] = $goodsName;
        $d['goodsDetail'] = $goodsDetail;
        $d['buyerName'] = $buyerName;
        $d['buyerContact'] = $buyerContact;
        $d['merRemark1'] = $merRemark1;
        $d['merRemark2'] = $merRemark2;
        $d['gopayServerTime'] = $gopayServerTime;

        $d['signValue'] = $signValue;
        $uid = $_SESSION['uid'];
        $data['bptime'] = time();
        $data['bptype'] ='充值';
        $data['uid'] =$uid;
        ...
        $data['isverified']=0;
        //以下是测试
       // echo "连接字符串：".$signStr;echo "<br/>";
       // echo "连接字符串md5:".$signValue;echo "<br/>";
       // dump($d);
       // dump($data);
        //exit;
        M('balance')->add($data);

       // $this->create($d, "https://gatewaymer.gopay.com.cn/Trans/WebClientAction.do");
        $this->create($d, "https://gateway.gopay.com.cn/Trans/WebClientAction.do");
        //智付接收地址
    }
		//微信支付接口
...
private function Mheader($type){
   ...
   public function reg()
    ...
    public function smsverify()
    {
        ...
        if(sendshortmessage($_POST['mobile'],"您的验证码是：".$code."。请不要把验证码泄露给其他人。")){
            die("提交成功");
        }else{
            die("短信发送失败");
        }
		...
		if(IS_POST){
			...
			if($bpwd['pwd']==md5($pwd)){
				if(strlen($banknumber)==16||strlen($banknumber)==19){
					...
					if($balance_result){
						// $accounts['balance'] = $bpwd['balance']-$bpprice1;
...
//注意调试生产环境时需要修改这个值为生产参数
        ...
        if($signValue==$signValue2){
            if($respCode=='0000'){
                ...
                //建议在此处进行商户的业务逻辑处理
                $parameter = array(
                    "order_no"     => $merOrderNum, //商户订单编号；
                    "order_amount"     => $tranAmt,    //交易金额；
                    "trade_status"     => 1, //交易状态
                );
                if(!checkorderstatus($merOrderNum)){
                    orderhandle($parameter);
                    recommend($merOrderNum,$tranAmt);
                    echo 'RespCode=0000|JumpURL=http://'.$_SERVER["HTTP_HOST"].'/index.php/index.php/Home/Index';
    ...
    public function rechargelist(){
        ...
        $balance = D('balance');
        $uid = $_SESSION['uid'];
        $where = array('uid'=>$uid,'bptype'=>'充值');
        $data = $balance->field('bpid,bpprice,remarks,bptime')->order('bpid desc')->where($where)->select();
    ...
    public function notify1(){
		...
		post('http://wei.ltbot.com/index.php/Home/User/notify.html',$abc);
    ...
    public function outsmsverify()
    {
		$phone = $_REQUEST['pho'];
        $code = $this->mescontent();
		$con = "你正在修改用户密码，验证码为".$code."！";
        include_once('sms.php');
        $row = sendshortmessage($phone,$con);
        if($row)
		{
			session("outcode",$code);
			echo 1;
		}else{
			echo 0;
		}
