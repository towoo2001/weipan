    ...
    public function ips(){
        ...
        $config = array(
            'head' => array(
                'Version' => 'v1.0.0', //版本号
                'MerCode' => '183375', //商户号 
                'MerName' => '宁夏蓝图商品交易市场有限公司', //商户名
                'Account' => '1833750019', //账户号
                // 'MsgId' => '', //消息编号
                'ReqDate' => date("YmdHis", time()), //商户请求时间
                // 'Signature' => '', //数字签名 boyd节点+商户号+数字签名
            ),
            'key' => "65uX7Edo671Krv4zRB2NsJFPWmZKEQkhVNJq5qwDNDJXzUEk41Bg0KbfPJsFCW2wh7MW8lRbNNjxwnPwdEUlddm8XgTZlOQzgUeSRPRQOxrmaZEReEwWyqq3J4MvxHvm", //数字签名
            'body' => array(
                'MerBillNo' => '10086001', //商户订单号 
                'Amount' => '', //订单金额 
                'Date' => date("Ymd", time()), //订单日期
                'CurrencyType' => '156',   //币种 
                'GatewayType' => '01', //支付方式 
                'Lang' => 'GB', //语言
                'Merchanturl' => 'http://' . $_SERVER['HTTP_HOST'] . '/index.php/Home/User/goipsNotice', //支付结果成 功返回的商 户URL 
                // 'FailUrl' => '', //支付结果失 败返回的商 户URL  
                // 'Attach' => '', //商户数据包 
                'OrderEncodeType' => '5', //订单支付接 口加密方式
                'RetEncodeType'=> '17', //交易返回接 口加密方式 
                'RetType' => '1', //返回方式 
                'ServerUrl' => 'http://' . $_SERVER['HTTP_HOST'] . '/home/pay/ipsReapalNotice', //异步S2S返 回 
                'BillEXP' => '1', //订单有效期 (小时)
                'GoodsName' => '充值', //商品名称
                // 'IsCredit' => '1', //直连选项 
                // 'BankCode' => '95533', //银行号 
        ...
        $data = array('Ips' => array(
         ...
         $data['Ips']['GateWayReq']['body']['MerBillNo'] = "HX" . time() . mt_rand(1000, 999999); //商戶訂單號
         $money = 10;//I('post.money');
         $data['Ips']['GateWayReq']['body']['Amount'] =  number_format($money, 2, ".", ""); //金额
        // $data['Ips']['GateWayReq']['head']['MsgId'] = 
         $data['Ips']['GateWayReq']['head']['Signature'] = md5( '<body>'.data_to_xml($data['Ips']['GateWayReq']['body']).'</body>'. $data['Ips']['GateWayReq']['head']['MerCode'] . $config['key'] );

        $uid = $_SESSION['uid'];
        $info['bptime'] = time();
        $info['bptype'] ='充值';
        $info['uid'] = $uid;
        $info['balanceno'] = $data['Ips']['GateWayReq']['body']['MerBillNo'];
        $info['remarks'] ="开始充值";
        $info['bpprice']= $money;
        $info['isverified']= 0;
        M('balance')->add($info);

        $this->assign('url', 'https://mobilegw.ips.com.cn/psfp-mgw/paymenth5.do');
        $this->assign('pGateWayReq',data_to_xml($data));
        $this->display();
    }	

    public function ipsReapalNotice(){
       $data = $_GET;
